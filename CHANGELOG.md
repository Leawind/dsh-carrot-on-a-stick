...
> 0.5.0 之后的全部工作合并为这一个版本发布。以下按开发内部批次（原标记 0.6.0→0.11.9）
> **时间倒序**组织（批次 1 的完整条目在本节末尾）；npm 只占用 0.6.0 一个版本号。

### 批次 15: session_list cwd 过滤(内部标记 0.11.9)

**`session_list` 支持 cwd 过滤**：传 `cwd` 参数只列该目录（含子目录，realpath 归一化比对）
下的会话——按项目维度找会话更直接。另加 `tools/list` 精确工具数守卫测试（13 个）。

### 批次 14: 纪要翻页(内部标记 0.11.8)

**`session_history` 支持翻页**：新增 `beforeIndex` 游标——传上次结果最早的 index 即可向更早
翻页，长会话可逐页回溯。

### 批次 13: 持久化原子写 + 架构图(内部标记 0.11.7)

**持久化原子写 + 架构图**：

- 队列持久化改为**原子写**：先落 `.tmp` 再 `rename`，`persistPath` 上的文件永远是完整
  JSON——进程在写入中途崩溃/断电不再留下半截文件（半截文件此前会让下次启动恢复失败，
  仅告警不阻断，但队列数据就丢了）。
- README 双语新增 `agent_run` 生命周期的 mermaid 时序图（进度心跳与取消路径一图看懂）。
- 冒烟测试 120 → 122 项（含"无 `.tmp` 残留"原子写断言）。

### 批次 12: 进度与续接体验(内部标记 0.11.6)
...
- 冒烟测试 117 → 121 项（含池新建会话纪要读取、GUI start 后真实握手闭环）。

### 批次 11: LRU 跳过活跃会话(内部标记 0.11.5)
