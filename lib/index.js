 ...
 */
let runTaskItem = () => { throw new Error('dsh-ops-mcp not applied'); };
...
export async function apply(ctx, config = {}) {
    ...
    const persistQueue = () => {
        ...
        persistChain = persistChain
            ...
            .catch((e) => console.warn('[dsh-ops-mcp] queue persist failed:', e?.message ?? e));
    };
    ...
    if (persistPath) {
        try {
            ...
            if (restored > 0)
                console.log(`[dsh-ops-mcp] queue restored from ${persistPath}: ${restored} items`);
        }
        catch (e) {
            if (e?.code !== 'ENOENT') {
                console.warn('[dsh-ops-mcp] queue restore failed:', e?.message ?? e);
            }
        }
    }
