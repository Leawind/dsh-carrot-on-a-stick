...
async function attachToWorkspace(ctx, canonical, sessionId) {
    try {
        ...
        console.warn('[dsh-carrot-on-a-stick] workspace attach failed:', e?.message ?? e);
    }
}
...
/** 正在执行 turn 的会话 id(池 LRU 淘汰时跳过——不能 dispose 一个活跃会话) */
const activeTurnSessions = new Set();
/**
 * 池 key = cwd + 模型三元组。
...
function knownSelectionOf(ctx, sessionId) {
...
async function getAgent(ctx, cwd, sessionId, title, override) {
    ...
    while (liveAgents.size >= runtimeConfig.maxAgents) {
        const evictable = [...liveAgents.entries()].find(([, rec]) => !activeTurnSessions.has(String(rec.sessionId)));
...
async function withLock(key, fn) {
    ...
    agentLocks.set(key, stored);
    void stored.then(() => {
        // 仍是链尾(没有后来的等待者替换掉它)才清; 有等待者时 map 里已是对方的 stored
        if (agentLocks.get(key) === stored)
            agentLocks.delete(key);
    });
    return next;
}
/**
 ...
 */
async function awaitIdleCancellable(agent, pending, signal, getCause = () => ({ kind: 'user' })) {
    const cancel = () => agent.cancel?.(getCause());
    if (!signal)
        return pending;
    if (signal.aborted) {
        cancel();
    ...
    let onAbort = () => { };
    const aborted = new Promise((resolveAborted) => { onAbort = () => { cancel(); resolveAborted(); }; });
    signal.addEventListener('abort', onAbort, { once: true });
    try {
        await Promise.race([pending, aborted]);
        if (signal.aborted)
            await pending; // cancel 已触发: 等真正收敛(而非半路返回)
    }
    finally {
...
function extractTexts(value, out = []) {
    if (Array.isArray(value)) {
        value.forEach((x) => extractTexts(x, out));
        return out;
    }
 ...
 */
function historyTurnsOf(events, limit, beforeIndex) {
    const turns = [];
    ...
    for (let i = start - 1; i >= 0 && turns.length < limit; i--) {
        ...
        if (ev?.type === 'assistant/message') {
            ...
            if (text.trim())
                turns.unshift({ index: i, role: 'assistant', text: clip(text, 600) });
        }
            ...
            if (texts.trim())
                turns.unshift({ index: i, role: 'tool_result', text: clip(texts, 300) });
        }
...
async function canonicalizeAllowedCwd(cwd) {
...
async function executeTask(opts) {
    ...
    return withLock(lockKey, async () => {
        ...
        if (reportProgress)
            void Promise.resolve(reportProgress(0)).catch(() => { });
        ...
        if (reportProgress) {
            progressTimer = setInterval(() => {
                try {
                    ...
                    void Promise.resolve(reportProgress(events)).catch(() => { });
                }
                catch { /* 忽略单次心跳失败 */ }
        ...
        try {
            ...
            activeTurnSessions.delete(String(sessionId));
        }
        // 结构化读输出
        const result = {
 ...
 */
let runTaskItem = () => { throw new Error('dsh-carrot-on-a-stick not applied'); };
let persistQueueHook = () => { };
...
async function reattachOrphanSessions(ctx) {
    ...
    for (const header of headers.values()) {
        ...
        try {
            ...
            failed++;
            console.warn(`[dsh-carrot-on-a-stick] 存量捞回失败 session ${header.id}:`, e?.message ?? e);
        }
    }
...
function registerTools(mcp, ctx) {
    ...
    mcp.registerTool('dsh_list_tools', {
    ...
    mcp.registerTool('task_inbox', {
        ...
        try {
            ...
            return outError(JSON.stringify({ error: e?.message ?? String(e) }));
        }
        const id = randomUUID();
        const item = {
    ...
    mcp.registerTool('task_result', {
        ...
        if (!item)
            return outError(JSON.stringify({ error: `task not found: ${taskId}` }));
    ...
    mcp.registerTool('session_list', {
        ...
        for (const session of sessions?.list?.() ?? []) {
            ...
            if (typeof session.title === 'string' && session.title)
                liveTitles.set(session.header.id, session.title);
        }
        const persistence = ctx.get('sessionPersistence');
        for (const snap of (await persistence?.list?.()) ?? []) {
            const header = headerOfSnapshot(snap);
    ...
    mcp.registerTool('session_history', {
    ...
    mcp.registerTool('select_model', {
        ...
        try {
            ...
            return outError(JSON.stringify({ error: `select_model failed: ${e?.message ?? String(e)}` }));
        }
    });
    // 给已有会话改名(走 sessionTitle 服务, 便于会话列表归档)
    mcp.registerTool('rename_session', {
        ...
        try {
            ...
            const session = sessions?.get?.(sessionId);
            if (!session)
                return outError(JSON.stringify({ error: `session not found: ${sessionId}` }));
            const st = ctx.get('sessionTitle');
            if (!st?.rename)
                return outError(JSON.stringify({ error: 'sessionTitle service unavailable' }));
    ...
    mcp.registerTool('attach_session', {
        ...
        try {
            ...
            const ws = await ensureWorkspace(ctx, canonical);
            if (!ws?.attachSession)
                return outError(JSON.stringify({ error: 'workspaceRegistry unavailable' }));
            ...
            return outError(JSON.stringify({ error: `attach failed: ${e?.message ?? String(e)}` }));
        }
    });
}
...
export async function apply(ctx, config = {}) {
    ...
    const nonNeg = (n, d) => Math.max(0, n ?? d);
    runtimeConfig = {
    ...
    const persistQueue = () => {
        ...
        persistChain = persistChain
            ...
            .catch((e) => console.warn('[dsh-carrot-on-a-stick] queue persist failed:', e?.message ?? e));
    };
    ...
    runTaskItem = (item) => {
        void (async () => {
            try {
                ...
                if (!item.sessionId && item.result.sessionId)
    ...
    if (persistPath) {
        try {
            ...
            if (restored > 0)
                console.log(`[dsh-carrot-on-a-stick] queue restored from ${persistPath}: ${restored} items`);
        }
        catch (e) {
            if (e?.code !== 'ENOENT') {
                console.warn('[dsh-carrot-on-a-stick] queue restore failed:', e?.message ?? e);
            }
        }
    }
    ...
    if (config.reattachOrphans === true) {
        void (async () => {
            try {
                ...
                console.warn('[dsh-carrot-on-a-stick] 存量捞回异常:', e?.message ?? e);
            }
        })();
    }
    ...
    const sessionSweeper = sessionTtl > 0
        ? setInterval(() => {
            ...
            for (const [sid, conn] of connections) {
                if (now - Math.max(conn.lastActivity, conn.connectedAt) <= sessionTtl)
                    continue;
