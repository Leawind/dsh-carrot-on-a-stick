...
function resolveAgentOptions(ctx) {
    ...
    throw new Error(`cannot determine model for spawned agent: plugin config has {provider: ${JSON.stringify(provider || null)}, model: ${JSON.stringify(model || null)}}, host default selection has {provider: ${JSON.stringify(def?.provider ?? null)}, model: ${JSON.stringify(def?.model ?? null)}}. `
...
function registerTools(mcp, ctx) {
    ...
    mcp.tool('agent_run', '同步执行任务(改代码/分析/跑命令), 返回结构化结果。可传 sessionId 续接已有会话(长任务分多轮投喂)。默认返回 summary 级(省上下文), 需要 toolCalls 原文时传 detail=full。', {
    ...
    mcp.tool('task_inbox', '把结构化任务(任务+上下文)推入 dsh 队列, 异步执行, 返回 taskId。', {
