...
| `agent_run` | 同步执行任务，返回结构化结果；`sessionId` 续接会话；`provider`/`model`/`reasoningEffort` 指定本次模型；`detail` 分级控制返回体积 |
...
- [ ] `agent_run` / `task_inbox` 无服务端超时与取消——卡死的 agent 会占住该 cwd 的串行锁，后续同目录任务排队等待；调用方需自带 MCP 层超时。队列也还不能列举/取消。
- [ ] 只读查询面还不完整：没有 `session_list`（列会话、读历史、查某会话当前模型），只有 `attach_session` / `rename_session` / `select_model`。
