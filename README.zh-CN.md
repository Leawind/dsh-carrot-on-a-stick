...
| `agent_run` | 同步执行任务，返回结构化结果；`sessionId` 续接会话；`provider`/`model`/`reasoningEffort` 指定本次模型；`detail` 分级控制返回体积；支持 MCP `notifications/cancelled` 取消（走宿主官方 `agent.cancel`） |
...
npm run smoke    # 端口 8099/8098/8096/8095/8094/8093/8092 假 ctx 冒烟(88 项, 真实 MCP 协议往返) + 端口冲突专项
```
